...
function Sales() {
  ...
  useEffect(() => {
    if (customerDueData?.data) {
      setCustomerDue(customerDueData.data);
    }
  }, [customerDueData]);
  ...
  const previousDue = customerDue?.outstandingDue || 0;
  const advanceBalance = customerDue?.advanceBalance || 0;
  const currentTotal = getTotal();
  const advanceUsed = Math.min(advanceBalance, currentTotal);
  const netPayable = currentTotal - advanceUsed;
  ...
  const openEditModal = (row) => {
    setEditingSale(row);
    setSelectedCustomerId(row.customer?._id || row.customer || '');
    setItems(
      row.items?.map((i) => ({
        product: String(
        ...
        <div class="invoice-container">
          ...
          <div class="parties">
            ...
            <div class="party" style="text-align:right">
              ...
              ${sale.status ? `<p style="margin-top:4px">Status: <span class="${sale.remainingDueAfterInvoice === 0 ? 'badge-paid' : sale.paymentReceivedAtInvoice > 0 ? 'badge-partial' : 'badge-due'}">${sale.remainingDueAfterInvoice === 0 ? 'Paid' : sale.paymentReceivedAtInvoice > 0 ? 'Partial' : 'Due'}</span></p>` : ''}
            </div>
          </div>
          ...
          <div class="payment-info">
            <div>
              ...
              <div class="value" style="margin-top:4px">
                <span class="${sale.remainingDueAfterInvoice === 0 ? 'badge-paid' : sale.paymentReceivedAtInvoice > 0 ? 'badge-partial' : 'badge-due'}">
                  ${sale.remainingDueAfterInvoice === 0 ? 'Paid in Full' : sale.paymentReceivedAtInvoice > 0 ? 'Partially Paid' : 'Unpaid'}
                </span>
              </div>
            </div>
            <div>
      ...
      <DataTable
        columns={[
          ...
          { header: 'Method', accessor: 'paymentMethod' },
          {
            ...
            accessor: 'status',
            cell: (row) => {
              if (row.remainingDueAfterInvoice === 0) return <Badge variant="success">Paid</Badge>;
              if (row.remainingDueAfterInvoice > 0 && row.paymentReceivedAtInvoice > 0)
            ...
            <Card className="bg-muted/30">
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-5 gap-4 text-sm">
                  ...
                  {advanceUsed > 0 && (
                    <div>
                      <p className="text-muted-foreground text-xs">Advance Used</p>
                      <p className="font-semibold text-green-600">-{formatCurrency(advanceUsed)}</p>
                    </div>
                  )}
                  <div>
        ...
        {detailModal && (
          <div className="space-y-4">
            ...
            <div className="space-y-1 border-t pt-4">
              ...
              {detailModal.advanceUsed > 0 && (
                <div className="flex justify-between text-sm">
                  <span>Advance Used</span>
                  <span className="text-green-600">-{formatCurrency(detailModal.advanceUsed)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span>Subtotal</span>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span>{formatCurrency(detailModal.paymentReceivedAtInvoice)}</span>
              </div>
