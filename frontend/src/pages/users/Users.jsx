...
function Users() {
  ...
  return (
    <div>
      <PageHeader title="Users" description="Manage system users and their roles" onAction={openCreate} />
      ...
      <DataTable
        columns={[
          ...
          { header: "Joined", accessor: "createdAt", cell: (row) => new Date(row.createdAt).toLocaleDateString() },
          {
            header: "Actions",
            accessor: "_id",
            sortable: false,
      ...
      >
        ...
        <FormField label="Email" name="email" error={errors.email?.message} required>
          <Input {...register("email")} type="email" placeholder="Email address" disabled={!!editingUser} />
        </FormField>
        <FormField
